...
# EBOTS: flat export from LeRobot (run scripts/export_lerobot_to_flat.py first)
# Expects output_dir/videos/episode_*.mp4 and output_dir/metas/episode_*.txt
_EBOTS_FLAT_DEFAULT = os.environ.get(
    "EBOTS_DATASET_DIR",
    str(Path.home() / "set_1_flat"),
)
example_video_dataset_ebots_448 = L(VideoDataset)(
    dataset_dir=_EBOTS_FLAT_DEFAULT,
    num_frames=93,
    video_size=(448, 448),
    caption_format="text",
)
dataloader_train_ebots_448 = L(get_generic_dataloader)(
...
example_video_dataset_ebots_224 = L(VideoDataset)(
    dataset_dir=_EBOTS_FLAT_DEFAULT,
    num_frames=93,
    video_size=(224, 224),
    caption_format="text",
)
dataloader_train_ebots_224 = L(get_generic_dataloader)(
...
# EBOTS: 2B, flat VideoDataset 448x448
predict2_video2world_training_2b_ebots_448 = dict(
    ...
    dataloader_train=dataloader_train_ebots_448,
    checkpoint=dict(
        save_iter=200,
        load_path=DEFAULT_CHECKPOINT_2B.s3.uri,
...
# EBOTS: 2B, flat VideoDataset 224x224
predict2_video2world_training_2b_ebots_224 = dict(
    ...
    dataloader_train=dataloader_train_ebots_224,
    checkpoint=dict(
        save_iter=200,
        load_path=DEFAULT_CHECKPOINT_2B.s3.uri,
        load_from_object_store=dict(enabled=False),
...
# EBOTS: 14B, flat VideoDataset 224x224
# torchrun --nproc_per_node=8 --master_port=12341 -m scripts.train --config=cosmos_predict2/_src/predict2/configs/video2world/config.py -- experiment=predict2_video2world_training_14b_ebots_224
predict2_video2world_training_14b_ebots_224 = dict(
    ...
    optimizer=dict(lr=2 ** (-15.5), weight_decay=0.001),
    scheduler=dict(
        f_max=[0.5],
        f_min=[0.2],
        warm_up_steps=[1_000],
    ...
    model_parallel=dict(context_parallel_size=1),
    model=dict(config=dict(state_t=12)),
)

# EBOTS: 14B, flat VideoDataset 448x448 (run: scripts/export_lerobot_to_flat.py first)
# torchrun --nproc_per_node=8 --master_port=12341 -m scripts.train --config=cosmos_predict2/_src/predict2/configs/video2world/config.py -- experiment=predict2_video2world_training_14b_ebots_448
